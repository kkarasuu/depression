...
st.write('Histogram that shows the level of education among interviewees')
fig = px.histogram(data, x="education_level")
st.plotly_chart(fig)
st.write('As we can seen, the most people have 10th level of education.')

# 2nd simple plot - scatter chart
st.write('Scatter chart that shows living expenses of respondents')
...
st.write('Now it is clear. We can clearly see the living expenses of respondents')
st.write('It also should be mentioned that less people spend more money')
fig = px.scatter(data, x="living_expenses", y="Survey_id")
st.plotly_chart(fig)

# 3rd simple plot - pie chart
st.write('This pie chart shows how many members are in families of respondents')
...
st.write('As it can be seen from this bar chart, the most depresive group is group of people who have 5 members in their families') 
...
st.write('Analyzing this violin chart, we can find that the most depressed people are among women of the middle age of 25 years')
...
st.write('If gender is 0, then it is male; if gender is 1, then it is female')
st.write('If depressed is 0, then a person not suffering from depression; if depressed is 1, then a person suffering  from depression')
